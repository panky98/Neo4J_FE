...
namespace DataLayer
{
    ...
    public static class DataProvider
    {
        ...
        public static object VratiSveZaposlen()
        {

            var obj = Session.Client.Cypher
                .Match("(f:Firma)<-[r:ZAPOSLEN]-(z:Zaposleni)")
                .Return((f, z, r) => new
                {
                    Firma = f.As<Firma>(),
                    Zaposleni = z.As<Zaposleni>(),
                    Zaposlen = r.As<Zaposlen>()
                })
                .Results.ToList();
        ...
        public static IList<Zaposleni> VratiSveDosadasnjeZaposleneDateFirme(int idFirme)
        {
            Dictionary<string, object> queryDict = new Dictionary<string, object>();
            queryDict.Add("id", idFirme);
            var query = new Neo4jClient.Cypher.CypherQuery("MATCH (n:Firma {id:{id}})<-[r:ZAPOSLEN]-(z:Zaposleni) return z;",
                                   queryDict, CypherResultMode.Set);
        ...
        //vrati trenutne zaposlene

        public static IList<Firma> VratiSveFirmeZaposlenog(int idZaposlenog)
        {
            Dictionary<string, object> queryDict = new Dictionary<string, object>();
            queryDict.Add("id", idZaposlenog);
            var query = new Neo4jClient.Cypher.CypherQuery("MATCH (f:Firma )<-[r:ZAPOSLEN]-(z:Zaposleni {id:{id}}) return f;",
                                   queryDict, CypherResultMode.Set);
        ...
        //vrati trenutne firme zaposlenog

        public static void DodajZaposlen(string nazivFirme, int idZaposlenog, Zaposlen zaposlen)
        {
            zaposlen.id = DataProvider.getMaxId() + 1;
            var query = Session.Client.Cypher
                        .Match("(f:Firma)", "(z:Zaposleni)")
                        .Where((Firma f) => f.naziv == nazivFirme)
                        .AndWhere((Zaposleni z) => z.id == idZaposlenog)
                        .Create("(f)<-[r:ZAPOSLEN {zaposlen}]-(z)")
                        .WithParam("zaposlen", zaposlen);

            query.ExecuteWithoutResults();
        }

        public static void izmeniZaposlen(Zaposlen zaposlen)
        {

            Dictionary<string, object> queryDictionary = new Dictionary<string, object>();

            queryDictionary.Add("id", zaposlen.id);
            queryDictionary.Add("pozicija", zaposlen.pozicija);
            queryDictionary.Add("datum_od", zaposlen.datum_od);

            CypherQuery query;

            if (zaposlen.datum_do != DateTime.MinValue)
            {
                queryDictionary.Add("datum_do", zaposlen.datum_do);

                query = new Neo4jClient.Cypher.CypherQuery("match (f:Firma)<-[r:ZAPOSLEN {id:{id}}]-(z:Zaposleni) SET r={id:{id}, datum_od:{datum_od}, datum_do:{datum_do}, pozicija:{pozicija}} return r",
                                                           queryDictionary, CypherResultMode.Set);
            }
            else
                query = new Neo4jClient.Cypher.CypherQuery("match (f:Firma)<-[r:ZAPOSLEN {id:{id}}]-(z:Zaposleni) SET r={id:{id}, datum_od:{datum_od}, pozicija:{pozicija}} return r",
                                                           queryDictionary, CypherResultMode.Set);
        ...
        public static void ObrisiZaposlen(int id)
        {
            ...
            var query = new Neo4jClient.Cypher.CypherQuery("match (f:Firma)<-[r:ZAPOSLEN {id:{id}}]-(z:Zaposlen) delete r",
                                                            queryDict, CypherResultMode.Set);

            ((IRawGraphClient)Session.Client).ExecuteCypher(query);
        }

        public static void ObrisiZaposlenZaDatuFirmu(int idFirme)
        {

            Dictionary<string, object> queryDict = new Dictionary<string, object>();
            queryDict.Add("id", idFirme);

            var query = new Neo4jClient.Cypher.CypherQuery("match (f:Firma {id:{id}})<-[r:ZAPOSLEN]-(z:Zaposleni) delete r",
                                                            queryDict, CypherResultMode.Set);

            ((IRawGraphClient)Session.Client).ExecuteCypher(query);
        }

        public static void ObrisiZaposlenZaDatogZaposlenog(int id)
        {
            ...
            var query = new Neo4jClient.Cypher.CypherQuery("match (f:Firma)<-[r:ZAPOSLEN]-(z:Zaposleni {id:{id}}) delete r",
                                                            queryDict, CypherResultMode.Set);
